...
int main(int argc, char *argv[])
{
	...
	{
		if (eq(Trim("  hi"), "hi") && eq(Trim("hi  "), "hi")
				&& eq(Trim("  hi  "), "hi"))
		{
...

	{
		...
		{
			std::cout << "Passing: "
					<< "ReplaceAll(\"monday morning\", \"mo\", \"kk\")"
					<< std::endl;
			pass_cases++;
		}
	}
	...
	{
		if (eq(ReadFile("docs/test_readfile.txt"), "12345"))
		{
			std::cout << "Passing: " << "ReadFile(\"test_readfile.txt\")"
					<< std::endl;
			pass_cases++;
		}
	}
	...
	{
		WriteFile("docs/test_writefile.txt", "abcde");
		if (eq(ReadFile("docs/test_writefile.txt"), "abcde"))
		{
			std::cout << "Passing: "
	...
	std::cout << "Result: " << pass_cases << "/12" << " Completed."
			<< std::endl;
